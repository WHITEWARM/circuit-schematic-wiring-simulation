...
[English](README.md) · **简体中文** · [下载技能 ZIP](https://github.com/WHITEWARM/circuit-schematic-wiring-simulation/releases/latest/download/circuit-schematic-wiring-simulation.zip) · [版本发布](https://github.com/WHITEWARM/circuit-schematic-wiring-simulation/releases) · [MIT 许可证](LICENSE)
...
快速安装：将下方内容发给 Codex，也可从 [最新版本](https://github.com/WHITEWARM/circuit-schematic-wiring-simulation/releases/latest) 下载已打包的技能。
...
下载 Release 中的 `circuit-schematic-wiring-simulation.zip`，解压后将完整技能文件夹复制到目标项目的 `.agents/skills/` 下。若下载的是完整仓库，则复制其中的 `skills/circuit-schematic-wiring-simulation` 文件夹。最终路径为：
...
38 项测试覆盖连接比较、几何审查、非法输入及报告文件保护。测试只验证两个检查工具及包结构，不等同于已经完成某个新电路的仿真、原生显示或自动布线验收。
...
如需反馈问题，请在 [Issues](https://github.com/WHITEWARM/circuit-schematic-wiring-simulation/issues) 中提供软件版本、预期与实际行为，以及最小复现示例，并区分工作流、检查脚本或仿真接口问题。
